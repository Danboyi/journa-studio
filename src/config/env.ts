...
const envSchema = z.object({
  ...
  AI_PROVIDER: z.enum(["claude", "openai", "demo"]).optional(),
  SENTRY_DSN: z.string().url().optional(),
  OPENAI_MODEL: z.string().default("gpt-4.1-mini"),
  OPENAI_EMBEDDING_MODEL: z.string().default("text-embedding-3-small"),
  ANTHROPIC_MODEL: z.string().default("claude-haiku-4-5-20251001"),
  ANTHROPIC_REFLECTION_MODEL: z.string().default("claude-sonnet-4-6"),
  ...
  COMPOSE_JOB_BATCH_SIZE: z.coerce.number().int().min(1).max(25).default(5),
...
export const env = envSchema.parse({
  ...
  ANTHROPIC_REFLECTION_MODEL: process.env.ANTHROPIC_REFLECTION_MODEL,
  ...
  COMPOSE_JOB_BATCH_SIZE: process.env.COMPOSE_JOB_BATCH_SIZE,
